...
void RenderManager::updateCameraUBO() {
    ...
    m_cachedCamera.position = m_currentCamera->getEye();
    m_cachedCamera.direction = m_currentCamera->getLook();
    m_cachedCamera.nearPlane = 0.1f; // Default near plane
    m_cachedCamera.farPlane = 1000.0f; // Default far plane
...
void RenderManager::updateTransformUBO(const glm::mat4& modelMatrix) {
    ...
    m_cachedMatrices.M = modelMatrix;
    m_cachedMatrices.MV = m_viewMatrix * modelMatrix;
    m_cachedMatrices.MVP = m_projMatrix * m_viewMatrix * modelMatrix;
    m_cachedMatrices.normalMatrix = glm::transpose(glm::inverse(glm::mat3(modelMatrix)));
    m_cachedMatrices.viewerPos = m_currentCamera ? m_currentCamera->getEye() : glm::vec3(0.0f);
    m_cachedMatrices.shouldNormalize = 1.0f;
...
void RenderManager::printFrameStats() {
    ...
    // Stats calculation maintained but output removed for cleaner console
    // Frame statistics are still tracked internally for debugging when needed
}
